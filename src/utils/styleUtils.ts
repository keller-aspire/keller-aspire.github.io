export type PaddingSize = 'xs' | 'small' | 'base' | 'large';
...
export const getPaddingClass = ({ padding, paddingTop, paddingBottom }: PaddingOptions): string => {
    ...
    if (padding) return paddingMap[padding];
    
    const topMap: Record<PaddingSize, string> = {
        'xs': 'pt-4',
        'small': 'pt-8',
        'base': 'pt-16',
        'large': 'pt-24'
    ...
    const bottomMap: Record<PaddingSize, string> = {
        'xs': 'pb-4',
        'small': 'pb-8',
        'base': 'pb-16',
        'large': 'pb-24'
    ...
    const classes = [];
    if (paddingTop) classes.push(topMap[paddingTop]);
    if (paddingBottom) classes.push(bottomMap[paddingBottom]);
    return classes.join(' ');
};
